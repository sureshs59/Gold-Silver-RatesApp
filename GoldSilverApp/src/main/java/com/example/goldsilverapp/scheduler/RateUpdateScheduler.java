...
public class RateUpdateScheduler {
    ...
    private void saveTodayRates() {
        try {
            ...
            log.info("Starting daily metal rate update for {}", today);
            ...
            for (ExternalRateResponse rate : rates) {
                ...
                log.info("Saved/Updated {} rate: {}", rate.getMetal(), rate.getPrice());
            }

            log.info("Daily metal rate update completed successfully");

        } catch (Exception ex) {
            log.error("Failed to update metal rates", ex);
        }
